...
Haushalt:
	Nachdem man in einen spezifischen Haushalt eingeloggt ist, können auf
	diesem Haushalt Befehle ausgeführt werden, sie Betreffen die konkrete
	Instanz des Haushaltes. Gast-Befehle sind immernoch möglich.
Benutzer:
	Nachdem man sich in ein Familienmitglied des Haushaltes eingeloggt hat,
	kann man die Benutzer-Befehle ausführen, sie werden auf der konkreten Instanz
	des Benutzers ausgeführt. Haushalts- und Gast-Befehle sind immernoch möglich.
...
Login
	...
	Meldet einen Haushalt an. Wenn der Login korrekt ist, betritt der Benutzer die
	Haushalts-Ebene und hat Zugriff auf Haushalts-Befehle.
...
LoginUser
	...
	Meldet einen Benutzer im Haushalt an. War der Befehl erfolgreich, wir die
	Benutzer-Ebene betreten und der Benutzer hat zugriff auf die Benutzer-Befehle.
	Der Rückgabewert "okay" ist true, wenn der Login erfolgreich war,
	sonst false.
...
ListUsers
	...
	Das Array beinhaltet Objekte mit dem Attribut "name", das den Namen des
	Benutzers darstellt.
...
CheckGrocery
	...
	Der Index ist der Index der Sache im Array. Es ist wichtig, dass die
	Indizes der Sachen im Array konstant bleiben.
	Mittels "GetGroceries" kann ein Array der aktuellen Einkaufsliste mit
	intakten Indizes ermittelt werden. Werden neue Einträge hinzugefügt, 
	so müssen diese UNTEN an das Array angefügt werden. Bei "ClearGroceries"
	wird aus dem Array gelöscht und alle Sachen rutschen auf.
...
UncheckGrocery
	...
	Return : { }
	Setzt den Status der Sache in der Einkaufsliste als "Nicht im 
	Einkaufswagen". Siehe auch: "CheckGrocery"
	
ClearGrocery
	Attribute: { }
	Return : { }
	Löscht alle sachen in der Einkaufsliste, deren Status "Im Einkaufswagen"
	ist (die checked sind). Siehe auch: "CheckGrocery"
...
AddRecipe
	...
	Return: { }
	Erstellt ein neues Rezept in der Datenbank. Es wird angenommen, dass 
	die Mengenangaben des Rezeptes für EINE PORTION gelten.
...
GroceryClear
	...
	Alle Einträge auf der Liste, die abgehakt sind (checked), MÜSSEN
	gelöscht werden. Um die Indizes des Arrays konstant und korrekt zu halten
	ist das löschen unumgänglich.
...
GroceryAdd
	...
	Sie sollte DANN UND ERST DANN dargestellt werden, der broadcast wird an alle,
	inklusive desjenigen, der das Event ausgelöst hat gesendet.
